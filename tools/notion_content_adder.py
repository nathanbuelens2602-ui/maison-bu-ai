...
def build_pages(refs):
    ...
    return {
        ...
        "Vandaag": [
            ...
            div(),
            h2("Posten vandaag"),
            p("Ga naar ", mention(db) if db else "Content Database",
              " → filter: Publicatiedatum = vandaag + Status ≠ Gepubliceerd")
              if db else p("Ga naar Content Database → filter: Publicatiedatum = vandaag + Status ≠ Gepubliceerd"),
            div(),
            h2("Goedkeuren"),
            p("Ga naar ", mention(db) if db else "Content Database",
              " → filter: Status = Wacht op goedkeuring")
              if db else p("Ga naar Content Database → filter: Status = Wacht op goedkeuring"),
            div(),
            h2("Afwerken"),
            p("Ga naar ", mention(db) if db else "Content Database",
              " → filter: Status = In productie + Deadline = vandaag")
              if db else p("Ga naar Content Database → filter: Status = In productie + Deadline = vandaag"),
            div(),
        ...
        "Deze Week": [
            ...
            p("Ga naar ", mention(db) if db else "Content Database",
              " → Board view, gegroepeerd op Dag van de week, gefilterd op Publicatiedatum = deze week.")
              if db else p("Ga naar Content Database → Board view, gegroepeerd op Dag van de week, gefilterd op Publicatiedatum = deze week."),
            div(),
            h2("Schema deze week"),
            table(
